...
class MazeEnv:
    ...
    def init_new_problem(self, index=None):
        ...
        # print ('self.obstacles', np.max(self.obstacles), np.min(self.obstacles))
        # self.obstacles = np.array(self.obstacles) / self.map.shape[0] - 0.5
        

        self.obstacles = np.array(self.obstacles) / self.map.shape[0] * 2 - 1.0
        # print ('normalized self.obstacles', np.max(self.obstacles), np.min(self.obstacles))
